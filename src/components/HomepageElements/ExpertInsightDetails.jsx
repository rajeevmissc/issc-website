...
import { FaChartLine, FaBalanceScale, FaLightbulb, FaUsers, FaCheckCircle, FaShareAlt, FaBell, FaFacebook, FaTwitter, FaLinkedin, FaInstagram } from "react-icons/fa";
...
const BlogPost = () => {
  ...
  if (!insight) {
    return <div className="text-center py-10 text-red-600 font-bold text-lg">Insight not found</div>;
  }

  return (
    <div className="max-w-3xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden my-12">
      
      {/* HERO SECTION */}
      <div className="relative h-80 bg-cover bg-center" style={{ backgroundImage: `url(${insight.authorImage})` }}>
      ...
      <div className="px-6 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          {insight.images.map((image, index) => (
            <img key={index} src={image} alt={`blog-${index}`} className="w-full h-52 object-cover rounded-lg shadow-md" />
          ))}
        </div>
        ...
        {/* Additional Insights */}
        <h2 className="text-xl font-semibold mb-2">{insight.additionalHeading}</h2>
        <p className="text-gray-700 leading-relaxed mb-4">{insight.additionalContent}</p>

        {/* Bullet Points with Icons */}
        <ul className="list-none space-y-3 mb-6">
          {insight.bulletPoints.map((point, index) => (
            <li key={index} className="flex items-center text-gray-700">
              {point.icon} <span className="ml-2">{point.text}</span>
            </li>
          ))}
        </ul>

        {/* Conclusion */}
        <h2 className="text-xl font-semibold mb-2">{insight.conclusionHeading}</h2>
        <p className="text-gray-700 leading-relaxed mb-6">{insight.conclusionContent}</p>

        {/* Social Media Links */}
        {/* <div className="flex justify-center space-x-4 mb-6">
          {insight.socialLinks.map((social, index) => (
            <a key={index} href={social.link} target="_blank" rel="noopener noreferrer" className="text-2xl">
              {social.icon}
            </a>
          ))}
        </div> */}
        ...
        {/* <div className="flex gap-4 justify-center">
          {insight.buttons.map((button, index) => (
            <button key={index} className="flex items-center px-5 py-2 bg-gray-300 text-gray-800 rounded-lg shadow-md hover:bg-gray-400">
